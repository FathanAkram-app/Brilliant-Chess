//! Tapered evaluation based on PeSTO (material + game-phase-interpolated
//! piece-square tables). Returns a score in centipawns from the perspective
//! of the side to move.
//!
//! Tables are written in "printed" order: index 0 = a8 ... index 63 = h1.
//! Our square indexing is a1 = 0 ... h8 = 63, so:
//!   white piece on square s reads table[s ^ 56]
//!   black piece on square s reads table[s]   (vertical mirror)
...
// Middlegame / endgame material values (P, N, B, R, Q, K).
const MG_VALUE: [i32; 6] = [82, 337, 365, 477, 1025, 0];
const EG_VALUE: [i32; 6] = [94, 281, 297, 512, 936, 0];

// Phase weights per piece type; total starting phase = 24.
...
pub fn evaluate(pos: &Position) -> i32 {
    ...
    for color in [Color::White, Color::Black] {
        ...
        for pt in ALL_PIECE_TYPES {
            ...
            while bb != 0 {
                ...
                bb &= bb - 1;
                // Table index: white mirrors with ^56, black reads directly.
                let idx = if color == Color::White { sq ^ 56 } else { sq };
                mg[c] += MG_VALUE[p] + MG_PST[p][idx];
                eg[c] += EG_VALUE[p] + EG_PST[p][idx];
                phase += PHASE_INC[p];
            }
        }
    }
    ...
    let white_score = (mg_score * phase + eg_score * (24 - phase)) / 24;
